...
Sources are processed in batches that keep the chunk material within `material_budget()`. Each chunk's progress is recorded in a run manifest under `.llm-wiki/runs/`, and the manifest is deleted only after the pages, page metadata, and per-file completion records are committed together. A failed batch leaves the manifest in place, so a retry resumes at the first unfinished chunk instead of reprocessing what already succeeded. A source whose bytes change while a run is open has its remaining chunks dropped from that run rather than sent against a different revision, and is planned again from its new content. A file is marked `complete` only when all of its chunks were processed and the update landed; a file that was skipped, deferred, or only partly processed keeps a status that plans it again on the next run.

A wiki written by an earlier version is upgraded once on first read: its file records become `unverified` and are re-ingested against their current content, and the previous `state.json` is kept at `.llm-wiki/state.v1.json`.
